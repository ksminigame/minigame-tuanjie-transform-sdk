...
public class KSProfileStatsScript : MonoBehaviour, KSWASM.KSSDKManagerHandler.KSProfileStatsScript
{
    ...
    public void OnEnable()
    {
        ...
        if (KSWASM.KSSDKManagerHandler.Instance.IsCloudTest())
        {
    ...
    public void Update()
    {
        ...
        UpdateValue("FrameTime(ms)", KSWASM.KSSDKManagerHandler.Instance.GetEXFrameTime(), sb, "0.00");
        ...
        UpdateValue("TotalHeapMemory", KSWASM.KSSDKManagerHandler.Instance.GetTotalMemorySize() / toMB, sb);
        UpdateValue("DynamicMemory", KSWASM.KSSDKManagerHandler.Instance.GetDynamicMemorySize() / toMB, sb);
        UpdateValue("UsedHeap(ProfilingMemory only)", KSWASM.KSSDKManagerHandler.Instance.GetUsedMemorySize() / toMB, sb);
        UpdateValue("UnAllocatedMemory", KSWASM.KSSDKManagerHandler.Instance.GetUnAllocatedMemorySize() / toMB, sb);

        sb.AppendLine("-------------AssetBundle----------");
        UpdateValue("NumberInMemory", KSWASM.KSSDKManagerHandler.Instance.GetBundleNumberInMemory(), sb);
        UpdateValue("NumberOnDisk", KSWASM.KSSDKManagerHandler.Instance.GetBundleNumberOnDisk(), sb);
        UpdateValue("SizeInMemory", KSWASM.KSSDKManagerHandler.Instance.GetBundleSizeInMemory() / toMB, sb);
        UpdateValue("SizeOnDisk", KSWASM.KSSDKManagerHandler.Instance.GetBundleSizeOnDisk() / toMB, sb);
    ...
    {   
        ...
        if(!KSWASM.KSSDKManagerHandler.Instance.IsCloudTest()) {
            ...
            {
                KSWASM.KSSDKManagerHandler.Instance.ProfilingMemoryDump();
            }
    ...
    {
        KSWASM.KSSDKManagerHandler.SetProfileStatsScript(typeof(KSProfileStatsScript));
    }
