...
// 判断是否是AssetBundle
unityNamespace.isKSAssetBundle = function (path) {
    return unityNamespace.KSAssetBundles.has(unityNamespace.PathInFileOS(path));
};
unityNamespace.PathInFileOS = function (path) {
    return path.replace(`${ks.env.USER_DATA_PATH}/__GAME_FILE_CACHE`, '');
};
unityNamespace.KSAssetBundles = new Map();
// 清理缓存时是否可被自动清理；返回true可自动清理；返回false不可自动清理
unityNamespace.isErasableFile = function (info) {
    // 用于特定AssetBundle的缓存保持
    if (unityNamespace.KSAssetBundles.has(info.path)) {
        return false;
    }
    // 达到缓存上限时，不会被自动清理的文件
...
// GameGlobal.realtimeLogManager = ks.getRealtimeLogManager();
// GameGlobal.logmanager = ks.getLogManager({ level: 0 });
GameGlobal.realtimeLogManager = {
    log: function (msg) {
        console.log("[RealtimeLogManager log]:", msg)
    },
    info: function (msg) {
        console.info("[RealtimeLogManager info]:", msg)
    },
    warn: function (msg) {
        console.warn("[RealtimeLogManager warn]:", msg)
    },
    error: function (msg) {
        console.error("[RealtimeLogManager error]:", msg)
    },
};
GameGlobal.logmanager = {
    log: function (msg) {
        console.log("[LogManager log]:", msg)
    },
    info: function (msg) {
        console.info("[LogManager info]:", msg)
    },
    warn: function (msg) {
        console.warn("[LogManager warn]:", msg)
    },
    error: function (msg) {
        console.error("[LogManager error]:", msg)
    },
};
// 提前监听错误并打日志
function bindGloblException() {
    ...
    //     }
    // });
    // ks.onUnhandledRejection((result) => {
    //     GameGlobal.realtimeLogManager.error(result);
    ...
    const systemInfoSync = ks.getSystemInfoSync();
...
GameGlobal.onCrash = GameGlobal.unityNamespace.onCrash = function () {
    ...
    const sysInfo = ks.getSystemInfoSync();
    // ks.createFeedbackButton({
    //     type: 'text',
    //     text: '提交反馈',
    //     style: {
