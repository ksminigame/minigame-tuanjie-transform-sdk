...
function requirePlugin (pluginString, pluginEnv, isSubContext = false) {
    const requirePath = `cachedPlugin/${pluginString}/index.js`

    window.pluginEnv = pluginEnv
    return require(requirePath)
}

GameGlobal.requirePlugin = requirePlugin
...
checkVersion().then((enable) => {
    if (enable) {
        ...
        try {
            ...
            UnityManager = requirePlugin('UnityPlugin', {
                ...
                customEnv: {
                    ksh: ks,
                    unityNamespace,
                    document: window.documentAdapter,
                    canvas,
                    events: GameGlobal.events,
                    KSWASMSDK: GameGlobal.KSWASMSDK,
                },
        ...
        GameGlobal.events.on('launchOperaPushMsgToWasm', (callback, args) => GameGlobal.KSWASMSDK.TJLaunchOperaBridgeToC(callback, args));
