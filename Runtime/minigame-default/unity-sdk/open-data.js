...
function getOpenDataContext() {
    return cachedOpenDataContext || ks.getOpenDataContext();
}
...
export default {
    KSDataContextPostMessage(msg) {
        getOpenDataContext().postMessage(msg);
    },
    KSShowOpenData(id, x, y, width, height) {
        if (width <= 0 || height <= 0) {
            console.error('[unity-sdk]: KSShowOpenData要求 width 和 height 参数必须大于0');
        }
