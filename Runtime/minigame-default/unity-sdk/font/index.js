...
const { platform } = ks.getSystemInfoSync();
...
function KSGetFontRawData(conf, callbackId) {
    const config = formatJsonStr(conf);
    const loadFromRemote = !GameGlobal.manager?.font?.getCommonFont;
    GameGlobal.manager.TimeLogger.timeStart('KSGetFontRawData');
    handleGetFontData(config).then(() => {
        if (fontDataCache) {
            GameGlobal.manager.font.reportGetFontCost(GameGlobal.manager.TimeLogger.timeEnd('KSGetFontRawData'), { loadFromRemote, isReadFromCache, preloadKSFont: GameGlobal.unityNamespace.preloadKSFont });
...
function KSShareFontBuffer(buffer, offset, callbackId) {
...
export function preloadTjCommonFont() {
    ...
    if (!!GameGlobal.unityNamespace.preloadKSFont && !!GameGlobal.manager?.font?.getCommonFont) {
        handleGetFontData();
    }
}
export default {
