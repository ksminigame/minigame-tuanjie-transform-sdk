...
function _JS_Video_Create(url) {
    ...
    if (isWebVideo) {
        ...
        if (cacheVideoDecoder.length > 0) {
            ...
            videoDecoder = ks.createVideoDecoder({
                type: 'wemedia',
            });
        }
        ...
        videoDecoder.on('start', (res) => {
            if (isDebug) {
                console.warn('ksVideoDecoder start:', res);
            }
        ...
        videoDecoder.on('stop', (res) => {
            if (isDebug) {
                console.warn('ksVideoDecoder stop:', res);
            }
            videoInstance.paused = true;
        });
        videoDecoder.on('seek', (res) => {
            if (isDebug) {
                console.warn('ksVideoDecoder seek:', res);
            }
        });
        videoDecoder.on('bufferchange', (res) => {
            if (isDebug) {
                console.warn('ksVideoDecoder bufferchange:', res);
            }
        });
