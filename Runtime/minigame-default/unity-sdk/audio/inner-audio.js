...
export default {
    // 创建audio对象
    KSCreateInnerAudioContext(src, loop, startTime, autoplay, volume, playbackRate, needDownload) {
    ...
    KSInnerAudioContextSetBool(id, k, v) {
        if (!checkHasAudio(id)) {
            return;
        }
    ...
    KSInnerAudioContextSetString(id, k, v) {
        if (!checkHasAudio(id)) {
            return;
        }
    ...
    KSInnerAudioContextSetFloat(id, k, v) {
        if (!checkHasAudio(id)) {
            return;
        }
    ...
    KSInnerAudioContextGetFloat(id, k) {
        if (!checkHasAudio(id)) {
            return 0;
        }
        return audios[id][k];
    ...
    KSInnerAudioContextGetBool(id, k) {
        if (!checkHasAudio(id)) {
            return false;
        }
        return audios[id][k];
    },
    KSInnerAudioContextPlay(id) {
        if (!checkHasAudio(id)) {
            return;
        }
    ...
    KSInnerAudioContextPause(id) {
        if (!checkHasAudio(id)) {
            return;
        }
        audios[id].pause();
    },
    KSInnerAudioContextStop(id) {
        if (!checkHasAudio(id)) {
            return;
        }
        audios[id].stop();
    },
    KSInnerAudioContextDestroy(id) {
        if (!checkHasAudio(id)) {
            return;
        }
        destroyInnerAudio(id, false);
    },
    KSInnerAudioContextSeek(id, position) {
        if (!checkHasAudio(id)) {
            return;
        }
    ...
    KSInnerAudioContextAddListener(id, key) {
        if (!checkHasAudio(id)) {
            return;
        }
        if (key === 'onCanplay') {
    ...
    KSInnerAudioContextRemoveListener(id, key) {
        if (!checkHasAudio(id)) {
            return;
        }
        audios[id][key]();
    },
    KSPreDownloadAudios(paths, id) {
        funs
            .downloadAudios(paths)
            .then(() => {
